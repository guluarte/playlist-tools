...
if len(sys.argv) > 1:
  ...
  print "You need to provide the name of the file without the extension. increments movie movie2\n"
  sys.exit(-1)
...
while current_time < total_seconds:
  playlist_str += get_track(separator1, 0, 2, False, vlc_id)
  vlc_id += 1
  playlist_str += get_track(movie, current_time, (current_time+INCREMENTS), False, vlc_id)
  vlc_id += 1
  playlist_str += get_track(separator2, 0, 2, False, vlc_id)
  vlc_id += 1
  playlist_str += get_track(movie, current_time, (current_time+INCREMENTS), False, vlc_id)
  vlc_id += 1
