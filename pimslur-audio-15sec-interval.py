...
while current_time < total_seconds:
  playlist_str += get_track(movie, current_time, (current_time+INCREMENTS), vlc_id)
  vlc_id += 1
  playlist_str += get_track(movie, current_time, (current_time+INCREMENTS), vlc_id)
  vlc_id += 1
  playlist_str += get_track(movie, current_time, (current_time+INCREMENTS), vlc_id)
  ...
  playlist_str += get_track(movie, current_time, (current_time+INCREMENTS), vlc_id)
  vlc_id += 1
  new_increment = current_time - 15
  if(new_increment >=0):
    playlist_str += get_track(movie, new_increment, (new_increment+INCREMENTS), vlc_id)
    vlc_id += 1
  new_increment = current_time - 30
  if(new_increment >=0):
    playlist_str += get_track(movie, new_increment, (new_increment+INCREMENTS), vlc_id)
    vlc_id += 1
  new_increment = current_time - 60
  if(new_increment >=0):
    playlist_str += get_track(movie, new_increment, (new_increment+INCREMENTS), vlc_id)
    vlc_id += 1
  new_increment = current_time - 120
  if(new_increment >=0):
    playlist_str += get_track(movie, new_increment, (new_increment+INCREMENTS), vlc_id)
    vlc_id += 1
  new_increment = current_time - 150
  if(new_increment >=0):
    playlist_str += get_track(movie, new_increment, (new_increment+INCREMENTS), vlc_id)
    vlc_id += 1
