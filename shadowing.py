...
def get_track(filename, start, end, subtitles, vlc_id, volume, no_audio):
  track_str = u"\t<track>\n"
  if subtitles:
  ...
  track_str += u"\t\t\t<vlc:option>volume=%s</vlc:option>\n" % volume
  track_str += u"\t\t\t<vlc:id>%s</vlc:id>\n" % vlc_id
  
  if no_audio:
	track_str += u"\t\t\t<vlc:option>noaudio</vlc:option>\n"
	track_str += u"\t\t\t<vlc:option>rate=0.80</vlc:option>\n"
...
if len(sys.argv) > 1:
  ...
  print "You need to provide the name of the file without the extension. increments movie\n"
  sys.exit(-1)
...
while current_time < total_seconds:
  playlist_str += get_track(movie, current_time, (current_time+INCREMENTS), subs, vlc_id, 1024, False)
  vlc_id += 1
  playlist_str += get_track(movie, current_time, (current_time+INCREMENTS), subs, vlc_id, 200, True)
  vlc_id += 1
  playlist_str += get_track(movie, current_time, (current_time+INCREMENTS), subs, vlc_id, 1024, False)
  vlc_id += 1
