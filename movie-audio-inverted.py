...
def get_track(filename, start, end, audio_track, vlc_id, slow):
  track_str = u"\t<track>\n"
  ...
  if slow:
      track_str += u"\t\t\t<vlc:option>rate=0.600000</vlc:option>\n"
...
while current_time < total_seconds:
  playlist_str += get_track(movie, current_time, (current_time+INCREMENTS), 2, vlc_id, False)
  vlc_id += 1
  playlist_str += get_track(movie, current_time, (current_time+INCREMENTS)+15, 1, vlc_id, True)
  vlc_id += 1
